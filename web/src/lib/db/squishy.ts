 ...
 *
 * SQUISHY_DATABASE_URL is optional in the env schema so the dashboard can boot
...
let _db: SquishyDb | null = null

function getDb(): SquishyDb {
  if (_db) return _db
  if (!env.SQUISHY_DATABASE_URL) {
    throw new Error(
      'SQUISHY_DATABASE_URL is required to use squishyDb — set it in the panel env',
    )
  }
  _client = postgres(env.SQUISHY_DATABASE_URL, { max: 5, idle_timeout: 30 })
  _db = drizzle(_client, { schema })
  return _db
}

export const squishyDb = new Proxy({} as SquishyDb, {
  get(_t, prop) {
